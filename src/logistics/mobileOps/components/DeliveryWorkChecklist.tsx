...
export default function DeliveryWorkChecklist({ acknowledgements, lpsConfirmed, readyForReceivingChecks, onToggle, onOpenReceivingChecks }: DeliveryWorkChecklistProps) {
  return (
    <Box sx={{ p: 1.5, borderRadius: 3, bgcolor: 'var(--active-theme-background-default)', border: '1px solid var(--paper-border-color)' }}>
      <Typography sx={{ color: 'var(--active-theme-text-primary)', fontSize: 15, fontWeight: 900 }}>Delivery work</Typography>
      <Typography sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 12.25, fontWeight: 600, lineHeight: 1.35, mt: 0.3 }}>Quick acknowledgements, not a receiving inspection checklist.</Typography>
...
function ChecklistRow({ label, description, complete, locked = false, onClick }: { label: string; description: string; complete: boolean; locked?: boolean; onClick?: () => void }) {
  return (
    <ButtonBase
      ...
      onClick={onClick}
      sx={{
        width: '100%', minHeight: 48, px: 0.5, display: 'flex', alignItems: 'center', gap: 1, borderRadius: 1.75, color: 'var(--active-theme-text-primary)', textAlign: 'left',
        '&:hover': onClick ? { bgcolor: 'var(--token-brand-soft-bg)' } : undefined, '&.Mui-disabled': { opacity: 1 }, '&:focus-visible': { outline: '2px solid rgba(29, 116, 255, 0.35)' },
      }}
    >
      {locked ? <LockOutlinedIcon aria-hidden="true" sx={{ color: 'var(--token-text-disabled)', fontSize: 20 }} /> : complete ? <CheckCircleIcon aria-hidden="true" sx={{ color: 'var(--token-success-main)', fontSize: 21 }} /> : <RadioButtonUncheckedIcon aria-hidden="true" sx={{ color: 'var(--token-text-disabled)', fontSize: 21 }} />}
      <Box sx={{ minWidth: 0, flex: 1 }}>
        <Typography component="span" sx={{ display: 'block', color: complete ? 'var(--token-success-main)' : 'var(--active-theme-text-primary)', fontSize: 12.75, fontWeight: 800, lineHeight: 1.25 }}>{label}</Typography>
        <Typography component="span" sx={{ display: 'block', color: 'var(--active-theme-text-secondary)', fontSize: 11.25, fontWeight: 600, lineHeight: 1.3, mt: 0.2 }}>{description}</Typography>
      </Box>
    </ButtonBase>
  );
}
