...
export default function UnloadingTaskDetail({ task, operatorName, onAssign, onUnassign, onStart, acknowledgements, confirmedPallets, onToggleAcknowledgement, onOpenPallets, onOpenReceivingChecks }: UnloadingTaskDetailProps) {
  ...
  return (
    <Stack spacing={1.5}>
      <Box>
        <Typography component="h2" sx={{ color: 'var(--active-theme-text-primary)', fontSize: 22, fontWeight: 900, lineHeight: 1.2 }}>Unloading task</Typography>
        <Typography sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 13.5, fontWeight: 600, lineHeight: 1.45, mt: 0.55 }}>Shared delivery work for the inbound team.</Typography>
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 2, borderRadius: 3, bgcolor: 'var(--active-theme-background-paper)', border: '1px solid var(--paper-border-color)', backgroundImage: 'none', boxShadow: 'var(--card-shadow)' }}>
        ...
        <Stack direction="row" spacing={1.25} sx={{ mt: 1.5 }}>
          <Box aria-hidden="true" sx={{ width: 44, height: 44, flex: '0 0 44px', display: 'grid', placeItems: 'center', borderRadius: 2.5, bgcolor: 'var(--token-brand-soft-bg)', color: 'var(--token-brand-main)', '& svg': { fontSize: 25 } }}><LocalShippingOutlinedIcon /></Box>
          <Box sx={{ minWidth: 0 }}>
            <Typography sx={{ color: 'var(--active-theme-text-primary)', fontSize: 17, fontWeight: 900, lineHeight: 1.25 }}>{task.vendor}</Typography>
            <Typography sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 13, fontWeight: 650, mt: 0.25 }}>{task.carrier} · Trailer {task.trailerId}</Typography>
          </Box>
        </Stack>

        <Divider sx={{ my: 1.75, borderColor: 'var(--surface-subtle-bg)' }} />
        <Stack spacing={1}>
        ...
        <Stack direction="row" alignItems="center" spacing={0.7}>
          <GroupsOutlinedIcon aria-hidden="true" sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 19 }} />
          <Typography sx={{ color: 'var(--active-theme-text-primary)', fontSize: 14, fontWeight: 850 }}>Assigned operators ({task.assignedOperators.length})</Typography>
        </Stack>
        {task.assignedOperators.length > 0 ? (
          <Stack spacing={0.55} sx={{ mt: 1 }}>
            {task.assignedOperators.map((operator) => (
              <Typography key={operator.name} sx={{ color: operator.name === operatorName ? 'var(--token-success-main)' : 'var(--active-theme-text-secondary)', fontSize: 13, fontWeight: operator.name === operatorName ? 850 : 650 }}>
                {operator.name === operatorName ? `${operator.name} (you)` : operator.name} · {operator.role}
              </Typography>
            ))}
          </Stack>
        ) : <Typography sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 13, fontWeight: 600, mt: 0.75 }}>No operators assigned yet.</Typography>}
      </Paper>
      ...
      {isAssignedToCurrentOperator && task.unloadingStatus === 'In progress' ? (
        <>
          ...
          <Button variant="outlined" onClick={onUnassign} sx={{ minHeight: 48, borderRadius: 2.5, borderColor: 'var(--paper-border-color)', color: 'var(--active-theme-text-primary)', fontWeight: 850 }}>Unassign</Button>
        </>
        ...
        <Paper elevation={0} sx={{ p: 1.5, borderRadius: 2.5, bgcolor: 'var(--active-theme-background-default)', border: '1px solid var(--paper-border-color)', color: 'var(--active-theme-text-secondary)' }}>
          <Typography sx={{ fontSize: 13.5, fontWeight: 750 }}>Waiting for coordinator release</Typography>
        </Paper>
      ) : task.unloadingStatus === 'Completed' ? (
        <Paper elevation={0} sx={{ p: 1.5, borderRadius: 2.5, bgcolor: 'var(--token-success-soft-bg)', border: '1px solid #9ACFBE', color: 'var(--token-success-main)' }}>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <Stack direction="row" justifyContent="space-between" spacing={2}>
      <Typography sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 12.5, fontWeight: 700 }}>{label}</Typography>
      <Typography sx={{ color: 'var(--active-theme-text-primary)', fontSize: 12.5, fontWeight: 800, textAlign: 'right' }}>{value}</Typography>
    </Stack>
  );
}

const primaryButtonSx = {
  minHeight: 48, borderRadius: 2.5, bgcolor: 'var(--token-brand-main)', color: 'var(--token-brand-contrast)', fontWeight: 850, boxShadow: 'none',
  '&:hover': { bgcolor: 'var(--token-brand-dark)', boxShadow: 'none' },
};
