...
export default function UnloadingTaskCard({ task, operatorName, onClick }: UnloadingTaskCardProps) {
  ...
  return (
    <ButtonBase
      ...
      sx={{
        width: '100%', display: 'block', p: 1.75, borderRadius: 3, border: '1px solid var(--paper-border-color)',
        bgcolor: 'var(--active-theme-background-paper)', color: 'var(--active-theme-text-primary)', textAlign: 'left', boxShadow: 'var(--card-shadow)',
        '&:hover': { borderColor: 'var(--token-brand-main)', boxShadow: '0 12px 28px rgba(11, 92, 171, 0.14)', transform: 'translateY(-1px)' },
    ...
    >
      <Stack direction="row" alignItems="flex-start" justifyContent="space-between" spacing={1}>
        ...
        <ChevronRightIcon aria-hidden="true" sx={{ color: 'var(--token-brand-main)', flexShrink: 0, mt: 0.15 }} />
      </Stack>

      <Stack direction="row" spacing={1.25} sx={{ mt: 1.25 }}>
        <Box aria-hidden="true" sx={{ width: 38, height: 38, flex: '0 0 38px', display: 'grid', placeItems: 'center', borderRadius: 2, bgcolor: 'var(--token-brand-soft-bg)', color: 'var(--token-brand-main)', '& svg': { fontSize: 22 } }}>
          <LocalShippingOutlinedIcon />
        </Box>
        <Box sx={{ minWidth: 0, flex: 1 }}>
          <Typography component="span" sx={{ display: 'block', color: 'var(--active-theme-text-primary)', fontSize: 15.5, fontWeight: 850, lineHeight: 1.25 }}>
            {task.vendor}
          </Typography>
          <Typography component="span" sx={{ display: 'block', color: 'var(--active-theme-text-secondary)', fontSize: 12.5, fontWeight: 650, lineHeight: 1.4, mt: 0.3 }}>
            {task.carrier} · {task.trailerId}
          </Typography>
        </Box>
      </Stack>

      <Typography component="span" sx={{ display: 'block', color: 'var(--active-theme-text-secondary)', fontSize: 12.5, fontWeight: 650, lineHeight: 1.4, mt: 1.25 }}>
        PO {task.purchaseOrder} · {task.dock} · {task.expectedPallets} expected pallets
      </Typography>
      <Typography component="span" sx={{ display: 'block', color: 'var(--active-theme-text-secondary)', fontSize: 12, fontWeight: 600, lineHeight: 1.4, mt: 0.35 }}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1} sx={{ mt: 1.25, pt: 1.15, borderTop: '1px solid #E5ECF2' }}>
        <Stack direction="row" alignItems="center" spacing={0.55} sx={{ minWidth: 0 }}>
          <GroupsOutlinedIcon aria-hidden="true" sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 18 }} />
          <Typography component="span" sx={{ color: 'var(--active-theme-text-secondary)', fontSize: 12.5, fontWeight: 750, lineHeight: 1.3 }}>
