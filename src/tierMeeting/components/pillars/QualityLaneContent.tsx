...
type QualityLaneContentProps = {
  pillar: TierMeetingPillar;
  settings: TierMeetingLaneSettings;
};

export default function QualityLaneContent({pillar, settings}: QualityLaneContentProps) {
    ...
    if (componentId === 'additionalCards' && visibleComponents.has(componentId)) {
      return (
        <Box key="additionalCards" sx={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 1}}>
          {[
            {value: '24', label: 'Days without quality issues', tag: 'Current'},
            {value: '89', label: 'Longest period', tag: 'Record'},
          ].map((item) => (
            <Paper
              key={item.label}
              elevation={0}
