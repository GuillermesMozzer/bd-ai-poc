...
type MyPeopleExpandedProps = {
  onBack: () => void;
  onCustomize?: () => void;
  visibleKpis?: string[];
};

const peopleSeries = tierChartMonths.map((month, index) => ({
  month,
  absense: reorderCalendarYearToTierYear([216, 222, 200, 249, 196, 239, 195, 216, 211, 195, 211, 216])[index],
  recognitions: reorderCalendarYearToTierYear([216, 222, 200, 249, 196, 239, 195, 216, 211, 195, 211, 216])[index],
  overtime: reorderCalendarYearToTierYear([196, 213, 210, 128, 215, 156, 213, 168, 132, 94, 164, 146])[index],
  gemba: reorderCalendarYearToTierYear([197, 213, 210, 127, 216, 157, 213, 167, 132, 94, 164, 146])[index],
}));

const metricCards = [
  {accent: tokenSuccess.darker, label: 'Absenteism', target: '2.8%', value: '2.8', suffix: '%'},
  {accent: tokenError.dark, label: 'Training', target: '100%', value: '83.3', suffix: '%'},
  {accent: tokenError.dark, label: 'Overtime', target: '100H', value: '141.4', suffix: 'h'},
];
      ...
      <Box sx={{display: 'flex', justifyContent: 'space-between', height: '100%'}}>
        <Box sx={{minWidth: 0}}>
          <Typography sx={{fontSize: 33, lineHeight: 0.95, color: isAlert ? red : workstationVisuals.tierTextHeading, fontWeight: 400}}>
            {value}
            <Box component="span" sx={{fontSize: 15, ml: 0.35, verticalAlign: 'super'}}>{suffix}</Box>
          </Typography>
        ...
        <Box sx={{pt: 1.1, textAlign: 'right'}}>
          ...
          <Box sx={{display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 41, height: 18, px: 0.55, mt: 0.15, borderRadius: 999, bgcolor: tokenNeutral.dark, color: workstationVisuals.tierTextHeading, fontSize: 10, fontWeight: 800}}>
            {target}
          </Box>
        </Box>
      </Box>
...
function PeopleBarChart({dataKey}: {dataKey: 'absense' | 'recognitions'}) {
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ReBarChart data={peopleSeries} margin={{top: 4, right: 18, left: -8, bottom: 8}}>
...
function PeopleLineChart({dataKey}: {dataKey: 'overtime' | 'gemba'}) {
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ReLineChart data={peopleSeries} margin={{top: 4, right: 22, left: -8, bottom: 9}}>
        <CartesianGrid vertical={false} stroke="transparent" />
        <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fontSize: 9, fill: workstationVisuals.tierTextHeading}} />
        <YAxis domain={[0, 300]} axisLine={false} tickLine={false} tick={{fontSize: 9, fill: workstationVisuals.tierTextHeading}} />
...
export default function MyPeopleExpanded({onBack, onCustomize, visibleKpis}: MyPeopleExpandedProps) {
      ...
      <Paper elevation={0} sx={shellSx}>
        <Box sx={{height: '100%', display: 'grid', gridTemplateRows: '28px minmax(0, 1fr)', rowGap: 0.8}}>
          <Box sx={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
            <Typography sx={{fontSize: 18, fontWeight: 900, color: workstationVisuals.tierTextHeading}}>People</Typography>
          ...
          <Box sx={{display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 320px', gap: 1.2, minHeight: 0}}>
            <Box sx={{position: 'relative', display: 'grid', gridTemplateRows: '66px 18px repeat(2, minmax(0, 366px))', gap: 1.1, minHeight: 0, height: '100%', pb: 5.2}}>
              ...
              <Box sx={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, minHeight: 0}}>
                {isKpiVisible('Absense') ? (
                  <Paper elevation={0} sx={{...cardSx, minHeight: 0}}>
                    <CardTitle title="Absense" />
                    <ChartFrame>
                      <PeopleBarChart dataKey="absense" />
                    </ChartFrame>
                  </Paper>
                ) : null}
                {isKpiVisible('Recognitions') ? (
                  <Paper elevation={0} sx={{...cardSx, minHeight: 0}}>
                    <CardTitle title="Recognitions" />
                    <ChartFrame>
                      <PeopleBarChart dataKey="recognitions" />
                    </ChartFrame>
                  </Paper>
                ) : null}
              </Box>
              ...
              <Box sx={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, minHeight: 0}}>
                {isKpiVisible('Overtime Hours') ? (
                  <Paper elevation={0} sx={{...cardSx, minHeight: 0}}>
                    <CardTitle title="Overtime Hours" />
                    <ChartFrame>
                      <PeopleLineChart dataKey="overtime" />
                    </ChartFrame>
                  </Paper>
                ) : null}
