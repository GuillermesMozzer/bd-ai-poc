...
import type {ForecastApprovalStatus, ForecastVersionType} from '../types';
...
export function ApprovalStatusBadge({status}: {status: ForecastApprovalStatus}) {
  const tone =
...
export function VersionTypeBadge({type}: {type: ForecastVersionType}) {
  return toneChip(type, type === 'Baseline' ? 'purple' : 'gray');
}
