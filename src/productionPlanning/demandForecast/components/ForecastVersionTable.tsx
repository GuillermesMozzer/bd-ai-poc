...
import type {CycleGroup, ForecastApprovalStatus, ForecastVersion} from '../types';
...
const STATUS_ORDER: ForecastApprovalStatus[] = ['Approved', 'Pending Approval', 'Rejected', 'Draft'];
const STATUS_COLORS: Record<ForecastApprovalStatus, string> = {
  Approved: '#027A48',
...
function CycleGroupHeader({
  ...
  const counts = STATUS_ORDER.reduce<Partial<Record<ForecastApprovalStatus, number>>>((acc, s) => {
      ...
      <TableCell colSpan={hasActionCol ? 10 : 9} sx={{py: 1, px: 1, border: 0}}>
        <Stack direction="row" alignItems="center" gap={1.5} flexWrap="wrap">
          ...
          <Box sx={{ml: 'auto', display: 'flex', gap: 1, flexWrap: 'wrap'}}>
            {(Object.entries(counts) as [ForecastApprovalStatus, number][]).map(([status, count]) => (
...
function VersionRow({
  ...
  v: ForecastVersion;
  isExpanded: boolean;
  onRowClick?: (v: ForecastVersion) => void;
  onToggleExpand: () => void;
}) {
  return (
    <>
      ...
      >
        ...
        <TableCell sx={bodyCellSx}>{v.cycleLabel}</TableCell>
        <TableCell sx={bodyCellSx}><VersionTypeBadge type={v.versionType} /></TableCell>
        ...
        <TableCell sx={bodyCellSx}><ApprovalStatusBadge status={v.approvalStatus} /></TableCell>
...
interface ForecastVersionTableProps {
  groups: CycleGroup[];
...
export default function ForecastVersionTable({
  groups,
  expandedId,
  expandedGroups,
  onRowClick,
  onToggleExpand,
  onToggleGroup,
}: ForecastVersionTableProps) {
  const totalVersions = groups.reduce((sum, g) => sum + g.versions.length, 0);

  if (totalVersions === 0) {
    return (
      <Paper elevation={0} sx={{...moduleCardSx, p: 4, textAlign: 'center'}}>
        <Typography sx={{color: 'var(--planning-text-muted)', fontSize: 14}}>No forecast versions match the current filters.</Typography>
      </Paper>
    );
  }
        ...
        <Table stickyHeader size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={headCellSx}>Cycle</TableCell>
