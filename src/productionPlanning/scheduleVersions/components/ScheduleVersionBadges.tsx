...
import type {ScheduleApprovalStatus, ScheduleVersionStatus} from '../types';
...
export function ScheduleApprovalStatusBadge({status}: {status: ScheduleApprovalStatus}) {
  const tone =
...
export function ScheduleStatusBadge({status}: {status: ScheduleVersionStatus}) {
  const tone =
    status === 'Published'   ? 'green'  :
    status === 'Frozen'      ? 'purple' :
    status === 'Superseded'  ? 'gray'   :
    status === 'Simulation'  ? 'indigo' : 'blue';
  return toneChip(status, tone);
}

export function ScheduleBaselineBadge() {
