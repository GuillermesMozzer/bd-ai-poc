...
import type {ScheduleVersionKpi} from '../types';
...
function KpiIcon({icon, tone}: {icon: ScheduleVersionKpi['icon']; tone: ScheduleVersionKpi['tone']}) {
  ...
  const iconEl =
    ...
    icon === 'versions' ? <CalendarMonthIcon sx={{fontSize: 20, color}} /> :
...
interface ScheduleVersionKpiCardsProps {
  cards: ScheduleVersionKpi[];
}

export default function ScheduleVersionKpiCards({cards}: ScheduleVersionKpiCardsProps) {
  return (
