...
import {scheduleVersions} from './mock';
import {ScheduleApprovalStatusBadge, ScheduleBaselineBadge, ScheduleStatusBadge} from './components/ScheduleVersionBadges';
import OrdersPlanningV2Page from '../ordersPlanningV2/OrdersPlanningV2Page';

interface ScheduleOrderCombinedPageProps {
  initialAssistantOpen?: boolean;
  initialVersionId?: string;
}

export default function ScheduleOrderCombinedPage({initialAssistantOpen = false, initialVersionId}: ScheduleOrderCombinedPageProps) {
  const [selectedVersion, setSelectedVersion] = useState<ScheduleVersion | null>(() =>
    initialVersionId ? (scheduleVersions.find((v) => v.id === initialVersionId) ?? scheduleVersions[0] ?? null) : null
  );
  ...
  return (
    <Box sx={{display: 'flex', flexDirection: 'column', gap: 0, bgcolor: 'var(--planning-background)', minHeight: '100%'}}>
      ...
      <OrdersPlanningV2Page initialAssistantOpen={initialAssistantOpen} />
    </Box>
  );
}
